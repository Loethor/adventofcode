...
fn solve_part1(input: &Vec<String>) -> usize {
    ...
    println!("Duration (incl reading from file): {:?}", duration);
    path_count
}
...
type CaveLookUpMap<'a> = HashMap<&'a str, usize>;

struct CaveSystem<'a> {
    caves: Vec<Cave>,
    cave_index_hm: CaveLookUpMap<'a>,
}
...
impl<'a> CaveSystem<'a> {
    fn new() -> Self {
        let mut cave_system = CaveSystem {
            caves: Vec::<Cave>::new(),
            cave_index_hm: CaveLookUpMap::<'a>::new(),
        };
    ...
    fn add_cave(&mut self, name: &'a str) {
        if self.cave_index_hm.contains_key(name) {
            return;
        }
        ...
        self.cave_index_hm.insert(name, cave_index);
    }
    ...
    fn add_connection_between_caves<'b: 'a, 'c: 'a>(
        ...
        let cave1_index = self.cave_index_hm[cave1_name];
        let cave2_index = self.cave_index_hm[cave2_name];
